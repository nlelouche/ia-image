...
public:
	...
	bool Create(DX_LPDIRECT3DDEVICE pDev, int id, bool bDynamic=true);
...
// ------------------------------------------------------------------------------
template <class PixelFormatClass, unsigned int FVF>
bool VertexBufferManager<PixelFormatClass, FVF>::Create(DX_LPDIRECT3DDEVICE pDev, int id, bool bDynamic)
{
	_id = id;
...
void VertexBufferManager<PixelFormatClass, FVF>::Flush(void)
{
	...
	if (_primitiveType == D3DPT_POINTLIST)
		...
		primitiveCount = _vtxToLock-2;
	else if (_primitiveType == D3DPT_TRIANGLEFAN)
		primitiveCount = _vtxToLock-2;
	else
		primitiveCount = 0;
